	...
	m_DemoPlayer(&m_SnapshotDelta, [&]() { UpdateDemoIntraTimers(); })
{
	...
	dbg_assert(mem_is_null(&m_CurrentServerPingUuid, sizeof(m_CurrentServerPingUuid)), "mem not null");
	...
	dbg_assert(mem_is_null(&m_Checksum, sizeof(m_Checksum)), "mem not null");
}
...
{
	CNetChunk Packet{};
	dbg_assert(mem_is_null(&Packet, sizeof(Packet)), "mem not null");
...
int CClient::HandleChecksum(int Conn, CUuid Uuid, CUnpacker *pUnpacker)
{
	...
	if(Start <= (int)sizeof(m_Checksum.m_aBytes))
	{
		...
		dbg_assert(mem_is_null(&m_Checksum.m_Data.m_Config, sizeof(m_Checksum.m_Data.m_Config)), "mem not null");
