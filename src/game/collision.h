...
public:
	...
	bool CheckPoint(fxpt<16, 16> x, fxpt<16, 16> y) const { return IsSolid(round_to_int(x), round_to_int(y)); }
	bool CheckPoint(vec2 Pos) const { return CheckPoint(Pos.x, Pos.y); }
	int GetCollisionAt(fxpt<16, 16> x, fxpt<16, 16> y) const { return GetTile(round_to_int(x), round_to_int(y)); }
	...
	void MovePoint(vec2 *pInoutPos, vec2 *pInoutVel, fxpt<16, 16> Elasticity, int *pBounces) const;
	void MoveBox(vec2 *pInoutPos, vec2 *pInoutVel, vec2 Size, vec2 Elasticity, bool *pGrounded = nullptr) const;
	bool TestBox(vec2 Pos, vec2 Size) const;

	// DDRace
	void SetCollisionAt(fxpt<16, 16> x, fxpt<16, 16> y, int Index);
	void SetDCollisionAt(fxpt<16, 16> x, fxpt<16, 16> y, int Type, int Flags, int Number);
	int GetDTileIndex(int Index) const;
	int GetDTileFlags(int Index) const;
	int GetDTileNumber(int Index) const;
	int GetFCollisionAt(fxpt<16, 16> x, fxpt<16, 16> y) const { return GetFTile(round_to_int(x), round_to_int(y)); }
	...
	int GetMoveRestrictions(CALLBACK_SWITCHACTIVE pfnSwitchActive, void *pUser, vec2 Pos, fxpt<16, 16> Distance = 18.0f, int OverrideCenterTileIndex = -1) const;
	int GetMoveRestrictions(vec2 Pos, fxpt<16, 16> Distance = 18.0f) const
	{
	...
	int GetPureMapIndex(fxpt<16, 16> x, fxpt<16, 16> y) const;
